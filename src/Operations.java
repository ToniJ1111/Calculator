...
public class Operations {
    ...
    public static void main(String[] args) {
        ...
        /*
        //Conditional Operators; And = &&: Executes only when right side is true, Or = ||: Executes Only when the left side is false
        int students = 150;
        int rooms = 0; 
        ...
        //not associated with if; group with block statement; {} must be used to create block statement
        else {
          diff = v2 - v1;
        ...
        //running multiple statemens within loops requires brackets
         do {
            ...
            System.out.print(" * 2 = "); //iVal = 5: 10 * 2 = 20; still < 25 run next loop
        ...
        float[] theVals = new float [3]; //each element accessed via index; range from 0 to number of elements -1; 
        theVals[0] = 10.0f;
        theVals[1] = 20.0f;
        theVals[2] = 15.0f;
        float sum = 0.0f; //variable to hold sum of array values
        //use loop to get a value of elements within the array;
        //set int to 1st index to access; add control condition (keep running as long as index is less than length of array; at end of pass, increment to next index)
        for(int index = 0; index < theVals.length; index++)
            //add value of each array element to sum variable
            sum += theVals[index];
            System.err.println(sum);//set index to 0;0< than length of array;0 is less than 3;add 10 t0 sum;update index from 0 to 1; 1<3 add 20 = 30 (10+20); 
            //index from 1 to 2;2<3, add 15 + 30 = 45; 3<3 is false; exit loop and print previous sum = 45.*/
        /*
